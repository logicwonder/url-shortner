...
func NewMongoRepository(mongoURL, mongoDB string, mongoTimeout int) (shortner.RedirectRepository, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("repository.NewMongoRepository: %w", err)
	}
	repo.client = client
	return repo, nil
}
...
func (r *mongoRepository) Find(code string) (*shortner.Redirect, error) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return nil, fmt.Errorf("repositiry.Redirect.Find: w", shortner.ErrRedirectNotFound)
		}
		return nil, fmt.Errorf("repositiry.Redirect.Find: w", err)
	}
	return redirect, nil
}
